...
namespace MonoBrickFirmware.Display
{
    public class Lcd : IDisposable
    {
        ...
        private const int bytesPerLine = (Width + 7) / 8;
        const int bufferSize = bytesPerLine * Height;
        byte[] displayBuf = new byte[bufferSize];

        private BmpImage screenshotImage = new BmpImage(bytesPerLine * 8, Height, ColorDepth.TrueColor);
        ...
        private LcdHAL _LcdHal = LcdHAL.Init;

        public void SetPixel(int x, int y, bool color)
        {
            int index = (x / 8) + y * bytesPerLine;
            int bit = x & 0x7;
            if (color)
        ...
        {
            // semaphore to control write access to be added later
            _LcdHal.Update(displayBuf, yoffset);
        }
        ...
        public void TakeScreenShot(string directory = "")
        {
            ...
            screenshotImage.WriteToFile(System.IO.Path.Combine(directory, "ScreenShot_") + string.Format("{0:yyyy-MM-dd_HH-mm-ss,fff}", DateTime.Now) + ".bmp");
        }

        public BmpImage GetScreen()
        {
            screenshotImage.Clear();
            float redActual = (float)endColor.Red;
            float greenActual = (float)endColor.Green;
        ...
        {
            Array.Clear(displayBuf, bytesPerLine * y, count * bytesPerLine);
        }
        ...
        {
            int bytePos = bytesPerLine * startPoint.Y + startPoint.X / 8;
        ...
        public void DrawBitmap(BitStreamer bs, Point p, uint xsize, uint ysize, bool color)
        {
            ...
            {
                int BufPos = bytesPerLine * yPos + p.X / 8;
