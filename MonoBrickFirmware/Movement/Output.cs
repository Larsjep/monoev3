...
namespace MonoBrickFirmware.Movement
{
	...
	internal class Output
	{
		...
		public void Start(){
			var command = new ByteArrayCreator();
			command.Append(KernelByteCodes.OutputStart);
			command.Append(BitField);
			pwmDevice.Write(command.Data);
		}
		...
		public void SetStepPower(sbyte power, UInt32 rampUpSteps, UInt32 constantSpeedSteps, UInt32 rampDownSteps, bool brake){
			WriteProfile(KernelByteCodes.OutputStepPower, power, rampUpSteps, constantSpeedSteps, rampDownSteps, brake);
		}
		...
		public void SetTimePower(byte power, UInt32 rampUpTime, UInt32 constantSpeedTime, UInt32 rampDownTime, bool brake){
			WriteProfile(KernelByteCodes.OutputTimePower,(sbyte) power, rampUpTime, constantSpeedTime, rampDownTime, brake);
		}
		...
		public void SetStepSpeed(sbyte speed, UInt32 rampUpSteps, UInt32 constantSpeedSteps, UInt32 rampDownSteps, bool brake){
			WriteProfile(KernelByteCodes.OutputStepSpeed, speed, rampUpSteps, constantSpeedSteps, rampDownSteps, brake);
		}
		...
		public void SetTimeSpeed(sbyte speed, UInt32 rampUpTime, UInt32 constantSpeedTime, UInt32 rampDownTime, bool brake){
			WriteProfile(KernelByteCodes.OutputTimeSpeed, speed, rampUpTime, constantSpeedTime, rampDownTime, brake);
		}
		...
		public void SetStepSync(sbyte speed, Int32 turnRatio, UInt32 steps, bool brake){
			var command = new ByteArrayCreator();
			command.Append(KernelByteCodes.OutputStepSync);
			command.Append(BitField);
			command.Append(speed);
		...
		public void ClearCount(){
			var command =  new ByteArrayCreator();
			command.Append(KernelByteCodes.OutputClrCount);
			command.Append(BitField);
			pwmDevice.Write(command.Data);
		}
